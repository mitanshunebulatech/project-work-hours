...
def list_leave_types(
    ...
    """
    Powers the Apply Leave dropdown. Only admins may request inactive types
    included — non-admins silently get the active-only list regardless of
    the query param, same "don't trust the client's role claim" posture
    used elsewhere (e.g. employee_id scoping in leave request search).
    """
    repo = LeaveTypeRepository(db)
    want_inactive = include_inactive and current_user.role == "admin"
